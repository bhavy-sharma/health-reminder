// app/patient/(protected)/queries/[id]/page.jsx
"use client";
...
import {
  ...
  XCircle, AlertCircle, MessageCircle, Image, File,
  Eye, X, Paperclip
} from 'lucide-react';
import toast, { Toaster } from 'react-hot-toast';
import Sidebar from '@/components/patient-dashboard/Sidebar';

export default function PatientQueryDetailPage() {
  ...
  const fetchQuery = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/patient/queries/${params.id}`);
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5]">
      ...
      <main className="md:pl-[280px]">
        <div className="p-4 sm:p-6 md:p-8 pb-20 max-w-[900px] mx-auto pt-16 md:pt-8">
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs font-medium opacity-70">
                          {msg.sender === 'patient' ? 'You' : msg.sender === 'doctor' ? 'Doctor' : 'Admin'}
                        </span>
                        <span className="text-[10px] opacity-50">
                          {new Date(msg.sentAt).toLocaleString()}
                        </span>
                      </div>
                      <p className={`text-sm ${msg.sender === 'patient' ? 'text-white/90' : 'text-gray-700'}`}>
                        {msg.message}
                      </p>
                              ...
                              >
                                {isImageFile ? (
                                  ...
                                  <div className={`w-12 h-12 rounded flex items-center justify-center shrink-0 ${
                                    msg.sender === 'patient' ? 'bg-white/20' : 'bg-gray-200'
                                  }`}>
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className={`text-xs font-medium truncate ${
                                    msg.sender === 'patient' ? 'text-white/90' : 'text-gray-700'
                                  }`}>
                                    {att.name}
                                  </p>
                                  <p className={`text-[10px] ${
                                    msg.sender === 'patient' ? 'text-white/50' : 'text-gray-400'
                                  }`}>
              ...
              <div className="p-6 border-t border-gray-100">
                ...
                <div className="flex gap-3">
                  <textarea
                    ...
                    rows="2"
                    className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0D1B2A] resize-none text-sm bg-white"
                    onKeyDown={(e) => {
