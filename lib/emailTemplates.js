...
export const getPatientVerificationEmailTemplate = ({ name }) => {
  const content = `
    <h3 style="color: #10B981;">Account Verified, ${name}!</h3>
    <p style="color: #333; line-height: 1.6;">We are pleased to inform you that your patient account on Family Health has been successfully verified by our administration team.</p>
    <p style="color: #333; line-height: 1.6;">You can continue using the platform to manage your family's health records and book appointments.</p>
    <div style="text-align: center; margin: 30px 0;">
      <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/login" style="background-color: #0D1B2A; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Log In</a>
    </div>
  `;
  return baseHtml(content);
};

export const getPatientSuspensionEmailTemplate = ({ name, reason }) => {
  const content = `
    <h3 style="color: #EF4444;">Account Suspended, ${name}</h3>
    <p style="color: #333; line-height: 1.6;">Your patient account on Family Health has been suspended by our administration team.</p>
    <div style="background-color: #FEF2F2; border-left: 4px solid #EF4444; padding: 15px; margin: 20px 0; border-radius: 0 4px 4px 0;">
      <p style="margin: 0; color: #7F1D1D; font-weight: bold;">Reason for Suspension:</p>
      <p style="margin: 10px 0 0 0; color: #7F1D1D; font-style: italic;">"${reason}"</p>
    </div>
    <p style="color: #333; line-height: 1.6;">If you believe this is a mistake, please contact our support team to resolve this issue.</p>
  `;
  return baseHtml(content);
};

export const getPatientDeletionEmailTemplate = ({ name, reason }) => {
  const content = `
    <h3 style="color: #EF4444;">Account Deleted, ${name}</h3>
    <p style="color: #333; line-height: 1.6;">Your patient account on Family Health has been permanently deleted by our administration team.</p>
    <div style="background-color: #FEF2F2; border-left: 4px solid #EF4444; padding: 15px; margin: 20px 0; border-radius: 0 4px 4px 0;">
      <p style="margin: 0; color: #7F1D1D; font-weight: bold;">Reason for Deletion:</p>
      <p style="margin: 10px 0 0 0; color: #7F1D1D; font-style: italic;">"${reason}"</p>
    </div>
    <p style="color: #333; line-height: 1.6;">Your account and associated data have been removed from the platform.</p>
  `;
  return baseHtml(content);
};

export const getPatientUnsuspensionEmailTemplate = ({ name }) => {
  const content = `
    <h3 style="color: #10B981;">Account Restored, ${name}!</h3>
    <p style="color: #333; line-height: 1.6;">We are pleased to inform you that the suspension on your Family Health patient account has been lifted by our administration team.</p>
    <p style="color: #333; line-height: 1.6;">You can now log in and resume using the platform to manage your family's health records and book appointments.</p>
    <div style="text-align: center; margin: 30px 0;">
      <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/login" style="background-color: #0D1B2A; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Log In</a>
    </div>
  `;
